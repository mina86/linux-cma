...
w~poprzednim podrozdziale są typy migracji (\code|migratetype|),
których jest sześć: \ang*{unmovable}, \ang*{reclaimable},
\ang*{movable}, \ang*{cma}, \ang*{reserve} oraz \ang*{isolate}).

\begin{itemize}
\item Dla potrzeb tej pracy traktuję typy \ang*{unmovable},
  \ang*{reclaimable} i~\ang*{reserve} jak jeden typ -- typ
...
\item Typ \ang*{cma} jest nowym typem dodanym dla potrzeb interfejsu
  CMA i~jest opisany dokładniej w~podrozdziale \ref{sec:migrate-cma}.
\item Typ \ang*{isolate} jest niejako pseudo-typem, gdyż jeżeli
