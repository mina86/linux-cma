...
Ponieważ mechanizm \acc{CMA} w~dużym stopniu integruje się
z~podsystemem zarządzania pamięci (\ang{memory management} lub
w~skrócie mm), do jego zrozumienia potrzeba ogólnej wiedzy na temat
tego, w~jaki sposób jądro śledzi i~przydziela pamięć procesom
i~sterownikom.
...
To uproszczenie wynika z~faktu, iż dla mechanizmu \acc{CMA} istotne jest
tylko rozróżnienie pomiędzy stronami ruchomymi i~nieruchomymi.

Typ \ang*{cma} jest nowym typem dodanym dla potrzeb interfejsu \acc{CMA}
...
\acc{RAM}.  Jednym z~przykładów są strony anonimowe działających procesów.
...
wyczyszczeniem pamięci \acc{TLB} (\ang{Translate Lookaside Buffer}), aby nie
...
\begin{algorithm}[p]
\caption[Alokacja z~uwzględnieniem list \acc{PCP}.]{Alokacja strony rzędu
  $k$ z~typem migracji $m$ z~uwzględnieniem list \acc{PCP}.}
\label{alg:buddy-pcp}
\begin{algorithmic}[1]
\Function{AllocPageUsePCP}{$k$, $m$}
    ...
    \Else
        \State $l \gets$ lista \acc{PCP} dla typu migracji $m$
        \If {$l = \emptyset$}
            \State $i \gets 0$
            \Repeat
        ...
        \Else
            ...
            \State lista \acc{PCP} dla typu migracji $m$ $\gets l$
        \EndIf
    \EndIf
    \State \Return $p$
\EndFunction
...
\section{Listy \acc{PCP}}\label{sec:pcp-lists}

Ostatnim istotnym, z~punktu widzenia mechanizmu \acc{CMA}, aspektem
alokatora stron są listy \acc{PCP} (\ang{per \acc{CPU} pages})
...
listy \acc{PCP}, na których znajdują się wolne strony rzędu 0.  Biorąc
...
istotne z~punktu widzenia mechanizmu \acc{CMA}.  Niemniej dla porządku
...
\item[ZONE\_DMA] Strefa przeznaczona dla urządzeń, które nie są
  w~stanie wykonywać transferów \acc{DMA} w~całej przestrzeni
  adresowej.  Na przykład w~architekturze x86 strefa ta jest
  przeznaczona dla urządzeń \acc{ISA}, które operują na 24-bitowych
  adresach.
...
jednego z~działających procesów, czego dokonuje \ang*{out-of-memory
  killer} lub \acc{OOM} \ang*{killer}) \autocite[rozdział 17]{bib:utlk}.
...
jednak ponieważ nie mają one wpływu na mechanizm \acc{CMA}, wychodzą
