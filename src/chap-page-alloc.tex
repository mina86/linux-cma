...
angielska nazwa: \textit{buddy system} lub \textit{buddy allocator}),
który operuje na blakach o~rozmiarze $2^k$ jednostek.  W~przypadku
...
których jest sześć: \textit{unmovable}, \textit{reclaimable},
\textit{movable}, \textit{cma}, \textit{reserve} oraz
\textit{isolate}).

\begin{itemize}
\item Dla potrzeb tej pracy traktuję typy \textit{unmovable},
  \textit{reclaimable} i~\textit{reserve} jak jeden typ -- typ
  nieruchomy.  To uproszczenie wynika z~faktu, iż dla mechanizmu CMA
  istotne jest tylko rozróżnienie pomiędzy stronami ruchomymi
  i~nieruchomymi.
...
\item Typ \textit{cma} jest nowym typem dodanym dla potrzeb interfejsu
  CMA i~jest opisany dokładniej w~podrozdziale \ref{sec:migrate-cma}.
\item Typ \textit{isolate} jest niejako pseudo-typem, gdyż jeżeli
  wolna strona ma taki typ, nie może ona zostać zaalokowana.  Więcej
  na temat sposobu w~jaki ten typ może być wykorzystywany opisuję
...
jednego z~działających procesów, czego dokonuje \textit{out-of-memory
  killer} lub OOM \textit{killer}) \autocite[rozdział 17]{bib:utlk}.
