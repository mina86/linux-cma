...
Alokator \acc{CMA} integruje się z~interfejsem programowania \acc{DMA} (\acc{DMA} \acc{API}),
co oznacza, że jeżeli mechanizm \acc{CMA} jest włączony i~zintegrowany
z~daną architekturą, poprawnie napisany sterownik (tzn.\ taki, który
korzysta z~\acc{DMA} \acc{API}) będzie korzystał z~alokatora \acc{CMA} bez konieczności
...
Dlatego w~architekturze \acc{ARM}, kod integrujący \acc{CMA} z~\acc{DMA} \acc{API} zmienia
