...
Idea mechanizmu \acc{CMA} jest stosunkowo prosta, jednak urósł on do
...
sterownika, który chciałby korzystać z~alokatora \acc{CMA} nie musi
niczego zmieniać w~swoim kodzie.

\section{Wykorzystanie w~sterownikach}\label{sec:usage-drivers}

Alokator \acc{CMA} jest zgodny z~interfejsem programowania \acc{DMA}
(\acc{DMA} \acc{API}), dzięki czemu jeśli mechanizm \acc{CMA} jest
włączony na danej architekturze, poprawnie napisany sterownik
(tzn.\ taki, który korzysta z~\acc{DMA} \acc{API}) będzie korzystał
z~alokatora \acc{CMA} bez konieczności dokonywania jakichkolwiek
zmian.
...
\begin{lstlisting}[float=tbhp,caption={Alokacja bufora pamięci z~użyciem
...
Dlatego w~architekturze \acc{ARM}, kod łączący \acc{CMA} z~\acc{DMA}
\acc{API} zmienia mapowanie stron na niebuforowane na czas, gdy są one
...
integruje mechanizm \acc{CMA} z~podsystemem \acc{DMA} architektury
