...
Wszystkie operacje \acc{PMM} działały w~czasie \BigO{\log n} co powodowało,
że \acc{PMM} skalował się nawet do tysięcy buforów i~wystarczał dla potrzeb
...
Głównymi zastrzeżeniami odnośnie \acc{PMM} było rezerwowanie na wyłączność
...
usuwanie (dodawanie) kości \acc{RAM} z~(do) komputera baz konieczności jego
wyłączania (\ang{memory hot-plugging})\,---\,i~nie należał do bardzo
stabilnych.  Kolejne wersje \acc{CMA} przynosiły iteracyjne poprawki
...
ten miał umożliwiać przenoszenie buforów \acc{CMA} jeżeli żadne urządzenie
...
poprzednimi wersjami \acc{CMA} okazało się oczywiste, że alokator musi si
bardziej integrować z~\acc{DMA} \acc{API} jądra, które jest standardowym
interfejsem, który sterowniki mogą wykorzystywać, by alokować bufory
\acc{DMA}.  Dlatego w~wersji dziesiątej \acc{CMA}, Marek Szyprowski, opiekun \acc{DMA}
\acc{API} dla architektury \acc{ARM}, dodał poprawki modyfikujące interfejs \acc{DMA}
\acc{ARM}-a tak, aby korzystał z~\acc{CMA}.  Osobiście byłem w~mniejszym stopniu
zaangażowany w~te zmiany pracując raczej głębiej w~podsystemie
zarządzania pamięcią.

Na skutek tej integracji funkcje \acc{CMA} przeznaczone dla sterowników
stały się niepotrzebne i~w~związku z~tym usunięte.  Począwszy od
wersji dziesiątej, sterowniki powinny korzystać z~\acc{DMA} \acc{API} i~nie
powinny być nawet świadome obecności \acc{CMA} w~systemie.  Pociągnęło to za
...
W~kwietniu 2012 roku 24 wersja alokatora \acc{CMA} została wydana, która
...
mechanizm \acc{CMA} był dostatecznie stabilne i~niezawodne, aby Andrew
...
drogę jaką przeszedł mechanizm \acc{CMA} łatwo zauważyć, jak jego zakres się
zmniejszał w~zamian zwiększając jakość rozwiązania.

Co więcej, prace nad mechanizmami \acc{PMM} i~\acc{CMA} pokazują, że współpraca
...
\acc{CMA}, muszę przyznać, iż praca nad dużym projektem wolnego
...
\end{tabular}
\end{center}
\caption{Afiliacje osób, które były wymienione w~ostatniej wersji \acc{CMA}.
