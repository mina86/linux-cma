...
brałem podczas pisania alokatora \acc{CMA}, jest wykorzystanie ich
...
w~systemie pamięci \acc{RAM} i~zamiast tego interpretuje przekazane
...
W~tej pracy opisuję alokator \acc{CMA} w~formie w~jakiej znalazł się on
w~Linuksie 3.5\footnote{Należy zauważyć, że Linux jest szybko
  rozwijającym się projektem wolnego oprogramowania i~ponieważ
  mechanizm \acc{CMA} używana jest przez coraz więcej osób, jest ona ciągle
...
Podobnie jak w~przypadku alokatora \acc{CMA}, wymaga to alokowania dużych
obszarów ciągłych fizycznie.  Tym co różni oba mechanizmy jest wymóg
aby alokacja \acc{CMA} zakończyła się sukcesem i~do tego w~jak najkrótszym
