...
Istotnie istnieją platformy sprzętowe z~układem \acc{MMU}
wejścia/wyjścia (\acc{IOMMU}), który to pozwala budować duże bufory
...
alokowania takich obszarów.\footnote{W~szczególności, Linux nie jest
  ...
  megabajtów, a~pojedyncza ramka \textit{full \acc{HD}} (tj.\ 1920
...
pamięci \acc{RAM} i~zamiast tego interpretuje dostarczone informacje.
...
W~trakcie moich prac stworzyłem menadżer pamięci fizycznej,
ang.\ \textit{Physical Memory Manager} (\acc{PMM})
\autocite{patch:pmm}, który implementuje dokładnie te założenia.
W~tym podstawowym zastosowaniu, \acc{PMM} nie przedstawia sobą niczego
...
Jednakże, pamięć zarezerwowana przez \acc{PMM} i~tak przez większość
czasu była zupełnie nieużywana.  Z~tego powodu, \acc{PMM} nie został
